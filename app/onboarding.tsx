    ...
    <View style={il.wrapper}>
      ...
      <View style={il.phone}>
        <View style={il.phoneScreen}>
          ...
          {/* Mini tx rows */}
          {['Kofi\'s Cafe  -₵45', 'Salary  +₵5,800', 'Uber Ride  -₵65'].map((t, i) => (
            <View key={i} style={il.txRow}>
              <View style={[il.txDot, { backgroundColor: i === 1 ? PRIMARY : '#F5A623' }]} />
      ...
      <View style={il.phone}>
        <View style={il.phoneScreen}>
          ...
          <View style={il.parsedCard}>
            ...
            <Text style={il.parsedRow}>Amount   <Text style={il.parsedVal}>₵ 50.00</Text></Text>
            <Text style={il.parsedRow}>Type     <Text style={il.parsedVal}>Send</Text></Text>
            <Text style={il.parsedRow}>Network  <Text style={il.parsedVal}>MTN</Text></Text>
          </View>
        </View>
      </View>
      {/* AI badge */}
        ...
        <View style={il.phoneScreen}>
          ...
          {nets.map((n) => (
            <View key={n.label} style={il.netRow}>
              <View style={[il.netBadge, { backgroundColor: n.bg }]}>
                <Text style={[il.netBadgeText, { color: n.text }]}>{n.label}</Text>
              </View>
              <View style={il.netBarBg}>
                <View style={[il.netBarFill, { width: n.pct, backgroundColor: n.text }]} />
              </View>
          ...
          <View style={il.checkRow}>
            <View style={[il.check, { backgroundColor: '#E1F5EE' }]}>
              <Text style={{ color: PRIMARY, fontWeight: '700', fontSize: 12 }}>✓</Text>
            </View>
...
const ILLUSTRATIONS = [WalletIllustration, AiIllustration, NetworksIllustration];
      ...
      <View style={ob.bottom}>
        ...
        <View style={ob.dots}>
          {SLIDES.map((_, i) => (
            <View
              key={i}
              style={[ob.dot, i === active && ob.dotActive]}
            />
          ))}
        </View>
...
const il = StyleSheet.create({
  ...
  balanceLabel: { fontSize: 9, color: 'rgba(255,255,255,0.75)', marginBottom: 2 },
  balanceAmt:   { fontSize: 16, fontWeight: '800', color: '#fff', marginBottom: 6 },
  ...
  smsTitle: { fontSize: 8, fontWeight: '700', color: '#92400E', marginBottom: 3 },
  ...
  parsedLabel: { fontSize: 8, fontWeight: '700', color: PRIMARY, marginBottom: 4 },
  parsedRow:   { fontSize: 7.5, color: '#1A1A1A', marginBottom: 2 },
  parsedVal:   { fontWeight: '700', color: PRIMARY },
  // Networks slide
  netHeading: { fontSize: 10, fontWeight: '700', color: '#1A1A1A', marginBottom: 8 },
  netRow:     { flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 5 },
  ...
  checkRow:   { flexDirection: 'row', alignItems: 'center', marginTop: 4, gap: 6 },
